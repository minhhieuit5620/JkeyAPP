...
public partial class ScanQR : ContentPage
{
    ...
    public ScanQR(IDeviceService deviceService, VerifyScanQRPageViewModel viewModel)
    {
        ...
        MainThread.BeginInvokeOnMainThread(async () =>
        {
            try
            {
                ...
                Dispatcher.Dispatch(async () =>
                {
                    ...
                    string value = args.Result[0].Text;
                    ...
                    if (Guid.Parse(uid) != Guid.Parse(configUserID))
                    {
                        ...
                        await Shell.Current.GoToAsync($"///{nameof(SmartOTPPage)}?VisibleView={false}&VisibleButton={true}");
                        return;
                    }
                    ...
                    if (checkScan.errorCode == "0")
                    {
                        ...
                        if (send)
                        {
                            ...
                            await Shell.Current.GoToAsync($"///{nameof(VerifyScanQR)}");
                        }
                        else
                        {
                            //await cameraView.StopCameraAsync();
                ...
                {                  
                    ...
                    Dispatcher.Dispatch(async () =>
                    {
                        ...
                        catch (FormatException)
                        {
                            ...
                            if (BindingContext is VerifyScanQRPageViewModel vnm)
                            {
                                vnm.Camera = true;
                                vnm.CameraOff = false;
                                vnm.IsBusy = false;
